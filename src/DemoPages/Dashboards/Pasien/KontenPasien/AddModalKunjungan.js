...
export class AddModalKunjungan extends React.Component {
    ...
    submitKunjungan = event =>  {
        ...
        const pasien = {
            ...
            idDokter: event.target.idDokter,
            idPasien: this.state.idPasien,
            idPraktek: event.target.idPraktek,
    ...
    axios.post("http://localhost:1212/v1/app/pasien", pasien)
        .then(response => {
    ...
    render() {
        let { alamat, berat_badan, idDokter,
            idPasien, idPraktek, jenis_kelamin,
            nama_lengkap, tanggal_lahir, tinggi_badan,
            username, usia } = this.state;
        return (
